function s=eom_draw(syst,varargin)
...
%  	s=[s  '<Viewpoint position="' pstn '"/>\n' ];
	...
	s=[s '<Transform translation="' pstn '">\n' item.x3d '</Transform>\n'];
...
for i=1:syst.data.nflex_points
	...
	if((item.forces==3||item.forces==0)&&(item.moments==3||item.moments==0))
		s=[s x3d_pnt(lcns,'rad',0.025,'col',[0 0 1])];
	else
		s=[s x3d_cyl([lcns-0.025*(item.axis)/norm(item.axis),lcns+0.025*(item.axis)/norm(item.axis)],'rad',0.02,'col',[0 0 1])];
	end
end
...
for i=1:syst.data.nrigid_points
	...
	lcns=item.location(:,1);
	if((item.forces==3||item.forces==0)&&(item.moments==3||item.moments==0))
		s=[s x3d_pnt(lcns,'rad',0.025,'col',[1 1 0])];
	else
		s=[s x3d_cyl([lcns-0.025*(item.axis)/norm(item.axis),lcns+0.025*(item.axis)/norm(item.axis)],'rad',0.02,'col',[1 1 0])];
	end
end
...
for i=1:syst.data.nsprings
	...
	x3d_spring=[x3d_spring x3d_pnt(item.location(:,1),'rad',2*link_rad,'col',[1;1;0])  ];  %% ball inner
	...
	x3d_spring_out=[x3d_spring_out x3d_cyl([item.location(:,2) temp1],'rad',link_rad,'col',[1;1;1])]; %% shaft

	frac=0.65+link_rad/len;
	temp2=frac*item.location(:,1)+(1-frac)*item.location(:,2);
	x3d_spring_out=[x3d_spring_out x3d_cyl([temp1 temp2],'rad',2.3*link_rad,'col',[1;1;1])];  %% piston

	x3d_spring_out=[x3d_spring_out x3d_pnt(item.location(:,2),'rad',2*link_rad,'col',[1;1;0])];  %% ball outer

	s=[s x3d_spring x3d_spring_out];
end

for i=1:syst.data.nlinks
	item=syst.data.links(i);
	lcns=[item.location(:,1) item.location(:,2)];
	s=[s x3d_cyl(lcns,'rad',0.015,'col',[0 1 0])];
end

for i=1:syst.data.nactuators
	item=syst.data.actuators(i);
	lcns=[item.location(:,1) item.location(:,2)];
	s=[s x3d_cyl(lcns,'rad',0.01,'col',[0 1 0.5])];
end

for i=1:syst.data.nsensors
	item=syst.data.sensors(i);
	lcns=[item.location(:,1) item.location(:,2)];
	s=[s x3d_cyl(lcns,'rad',0.01,'col',[0 0.5 1])];
end

for i=1:syst.data.nbeams
	item=syst.data.beams(i);
	lcns=[item.location(:,1) item.location(:,2)];
	s=[s x3d_cyl(lcns,'rad',0.02,'col',[1 0.5 0])];
end
...
for i=1:syst.data.nloads
	...
	s=[s x3d_cyl([lcns,lcns+dirn],'rad',0.01,'col',[0 1 1])];
	s=[s x3d_cyl([lcns+dirn,lcns+1.5*dirn],'rad',0.015,'cone','col',[0 1 1])];
	...
	s=[s x3d_cyl([lcns,lcns+dirn],'rad',0.01,'col',[0 1 1])];
	s=[s x3d_cyl([lcns+dirn,lcns+1.5*dirn],'rad',0.015,'cone','col',[0 1 1])];
	s=[s x3d_cyl([lcns+1.3*dirn,lcns+1.8*dirn],'rad',0.015,'cone','col',[0 1 1])];
end
...
for i=1:syst.data.nnh_points
	...
	lcns=item.location(:,1);
	if((item.forces==3||item.forces==0)&&(item.moments==3||item.moments==0))
		s=[s x3d_pnt(lcns,'rad',0.025,'col',[1 1 0.5])];
	else
		s=[s x3d_cyl([lcns-0.03*(item.axis)/norm(item.axis),lcns+0.03*(item.axis)/norm(item.axis)],'rad',0.015,'col',[1 1 0.5])];
	end
end
...
for i=1:syst.data.ntriangle_3s
	...
	lcns=item.location;
	s=[s x3d_ifs([0;0;0],[0;0;1;0],lcns,[0;1;2;-1],'col',[1 0 0.5],'tran',0.5)];
	avg=(lcns(:,1)+lcns(:,2)+lcns(:,3))/3;
	s=[s x3d_cyl([avg  avg+0.05*item.unit], 'col',[1 0 0.5])];
	s=[s x3d_cyl([avg  avg+0.05*item.nu(:,1)], 'col',[1 0 0.5])];
	s=[s x3d_cyl([avg  avg+0.05*item.nu(:,2)], 'col',[1 0 0.5])];

end

x3d_save(s,[syst.config.dir.output filesep() 'x3d' filesep() 'system.x3d']);
if(verb) disp('Drawing x3d done.'); end
