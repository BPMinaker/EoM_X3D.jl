...
x3d_tags=["ground" "upright" "strut" "arm" "wheel" "chassis" "bell" "rack" "rod" "bar"]  ## Define significant body names
...
for i in syst.bodys  ## Loop over each body
	...
	for j=1:length(x3d_tags)  ## Loop over each sig. name
		if occursin(x3d_tags[j],lowercase(i.name))  ## If the body name contains a sig. name, replace the default string
			...
			elseif j==6 #"chassis"
				x3d=x3d_pnt([0,0,0],cubes=true,rad=[0.8,0.3,0.5],col=[0,0,0.5],tran=0.3)

			elseif j==7 #"bell"
				x3d=x3d_pnt([0,0,0],cubes=true,rad=[0.04 0.04 0.04],col=[0.3,0.3,0.6])
