...
for j=1:nbodys
	...
	if (~strcmp(name,'ground'))  %% If we have a body that's not ground, cause the ground doesn't move
		...
		s1=[s1 x3d_motion(tout,temp1,temp2,ones(3,size(pout,1)),[num2str(j) '_body'],syst.data.bodys(j).x3d)]; %% Add the x3d string for the motion indicated by the mode shape
	else
		s1=[s1 syst.data.bodys(j).x3d];  %% Ground doesn't move
	end
end
...
for j=1:syst.data.nrigid_points
	...
	s1=[s1 x3d_motion(tout,lcn,rot,ones(3,size(pout,1)),[num2str(j) '_rigid_point'],x3d)]; %% Add the x3d string for the motion indicated by the mode shape
...
for j=1:syst.data.nflex_points
	...
	s1=[s1 x3d_motion(tout,lcn,rot,ones(3,size(pout,1)),[num2str(j) '_flex_point'],x3d)]; %% Add the x3d string for the motion indicated by the mode shape
...
for j=1:syst.data.nlinks
	...
	s2=[s2 x3d_motion(tout,lcn1,aa,scale,[num2str(j) '_link'],x3d_link)]; % insert the picture of the arm into the animation data
end
...
%	len=norm(syst.data.springs(j).location1-syst.data.springs(j).location2);
	...
	x3d_spring=[x3d_spring x3d_pnt([0;0;0],'rad',2*link_rad,'col',[1;1;0])  ];

	len=len*.65;
	x3d_spring_out=x3d_cyl([[0;2*link_rad;0] [0;len;0]],'rad',3*link_rad,'tran',0.3,'col',[1;1;0]);
	x3d_spring_out=[x3d_spring_out x3d_cyl([[0;0;0] [0;len+link_rad;0]],'rad',link_rad,'col',[1;1;1])];
	x3d_spring_out=[x3d_spring_out x3d_cyl([[0;len-link_rad;0] [0;len+link_rad;0]],'rad',2.3*link_rad,'col',[1;1;1])];
	x3d_spring_out=[x3d_spring_out x3d_pnt([0;0;0],'rad',2*link_rad,'col',[1;1;0])];
	...
	s3=[s3 x3d_motion(tout,lcn1,aa1,ones(3,size(pout,1)),[num2str(j) '_spring'],[x3d_spring])]; % insert the picture of the arm into the animation data
	s4=[s4 x3d_motion(tout,lcn2,aa2,ones(3,size(pout,1)),[num2str(j) '_spring_out'],x3d_spring_out)];
end
