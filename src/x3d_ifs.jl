...
for i=1:n
	...
	s=[s '<Transform translation="' pstn_s '"  rotation="' rotn_s '" >\n'];
	s=[s ' <Shape>\n'];
	s=[s '  <Appearance>\n'];
	s=[s '   <Material  emissiveColor="' color '" diffuseColor="' color '" ' tran_s '/>\n'];
	s=[s '  </Appearance>\n'];
	s=[s ' <IndexedFaceSet coordIndex="'];
	for j=1:size(faces,2)
		s=[s sprintf('%i %i %i %i %i ',faces(:,j))];
	end
	s=[s '">\n'];
	s=[s '  <Coordinate point="'];
	for j=1:size(vert,2)
		s=[s sprintf('%f %f %f ',vert(:,j))];
	end
	s=[s '"/>\n'];
	s=[s ' </IndexedFaceSet>\n'];
	s=[s ' </Shape>\n'];
